...
//	}
	...
	//가로길이(width)와 세로길이(height)에 전달하면 코드 중복 제거
	public double cal(double width) {
		//cal(width, height)메서드를 호출하여
		//가로 길이를 두 매개변수에 전달하면 직사각형 계산과 거의 동일하므로
		//계산된 결과를 리턴받아 다시 리턴
		return cal(width, width);
	}
	//사각형 가로, 세로 길이를 전달받아 직사각형 넓이 계산
	public double cal(double width, double heigth) {
		System.out.println("사각형 가로 길이 = " + width);
		System.out.println("사각형 세로 길이 = " + heigth);
		return width * heigth;
	}
}