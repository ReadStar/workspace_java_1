...
public class Ex_Stack {
	...
	public static void main(String[] args) {
		 ...
		 * 	 =마지막에 들어간 객체가 먼저 나오는 구조(Last In First Out)
		 *  =후입선출 구조
		 * -주로 응용프로그램의 undo/redo, 웹브라우저의 뒤로/앞으로 기능을
		 *   2개의 Strack(backward, forward)으로 구현
		 */
		...
		//isEmpty() 메서드 결과가 false일 때만 꺼내기 수행
		if(!stack.isEmpty()) {
		 ...
		 * -한 쪽에서 offer()로 삽입, 반대쪽에서 poll()로 삭제(꺼내기)
		 * -구현체 클래스 : LinkedList
		 * 	 =>LinkedList 클래스는 Queue와 List 인터페이스 모두 구현
		 * -은행 업무 번호표, 최근 문서 구현에 사용
		 */
		Queue q = new LinkedList();
		q.offer("1-Ex.Java");
		q.offer("2-main.jsp");
		q.offer("3-index.html");
		System.out.println("최근문서목록 : " + q);
		...
		//실제 사용 시 Queue의 크기를 제한해두고
		//offer()로 삽입할 때 Queue의 크기가 다 찼을 경우
