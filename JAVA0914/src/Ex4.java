...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * -프로그램에서 다 하나뿐인 유일한 객체(인스턴스) = 싱글톤 객체
		 * -싱글톤 객체를 사용하여 프로그램을 작성하는 기법을
		 * 	  싱글톤 디자인 패턴이라고 한다
		 * -새로운 인스턴스 새성이 불가능하게 하며,
		 ...
		 * 		=>참조변수를 static변수로 선언하여 객체 생성 없이 로딩
		 * 3. 새성된 인스턴스를 외부로 리턴하는 Getter를 정의
		 * 		=>static 변수를 리턴하므로 Getter 메서드도 static으로 선언
		 */
		
		//접근제한자가 Private로 선언된 생성자 호출이 불가능하므로
		//SingleClass의 인승턴스 생성이 불가능해짐
//		SingletonClass sc = new SingletonClass();
		
		//sataic으로 선언된 정적 멤버변수 instance에 접근하여
...
class SingletonClass{
	...
	//	   외부에서 인스턴스 생성(생성자 호출이 불가능하도록)을 못하도록 제한
