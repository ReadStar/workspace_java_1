...
public class Ex3 {
	...
	public static void main(String[] args) {
		...
		gm.<String>genericMethoid1("홍길동");
		gm.<Integer>genericMethoid1(10);
		
		int num = gm.<Integer>genericMethoid2(20);
		System.out.println("리턴받은 데이터 : " + num);
		
		String str = gm.<String>genericMethoid2("홍길동");
		...
		//extends Object와 동일하게 취급되므로 모든 타입을 제네릭 지정 가능
...
//		wgt.method3(new GenericBox3<SpiderMan>());
		//Person 또는 Person 클래스의 슈퍼클래스 타입(Obejct)만 가능
		wgt.method3(new GenericBox3<Object>());
		
		System.out.println("------------------------------------");
		
		//제네릭타입을 메서드 파라미터에서 제한하는 예
...
class GenericMethod{
	...
	public <P> void genericMethoid1(P p) {
		//메서드 파라미터로 전달받을 데이터타입을 제네릭타입 P로 지정
		//=>리턴타입 앞에 제네릭타입 <P> 지정 필수
		System.out.println("파라미터 타입" + p.getClass().getName());
		System.out.println("전달받은 데이터 : " + p);
	}
	public <P> P genericMethoid2(P p) {
		System.out.println("파라미터 타입" + p.getClass().getName());
		System.out.println("전달받은 데이터 : " + p);
		return p;
	}
