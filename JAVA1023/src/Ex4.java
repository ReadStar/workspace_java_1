...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 * -예외가 발견되는 시점에 따라 Compile Checked Exception 계열과
		 * 	 Compile unchecked Exception 계열로 나뉨
		 * 	 1)Com[ile Checked Exception : 컴파일 시점(코드 작성 시점)에서
		 * 		 예외 처리 여부를 판단하여 예외가 처리되지 않으면 예외 발생 알림
		 * 		 ex) IOException, SQLException, FileNotFoundException 등
		 * 	 2)Compile Unchecked Exception : 컴파일 시점에서는 예외처리 여부를
		 * 	 	 검사하지 않고, 실행 시점에서 예외가 발생하면 예외 발생 알림
		 * 		 ex) RuntimeException 계열(ArithmeticException 계열)
		 * 
		 ...
		 *			=>null 이 저장된 참조 변수의 .XXX() 형태로 메서드 호출 시
