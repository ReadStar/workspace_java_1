...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * 		  진행중이더라도 다른 작업을 대기하지 않고 번갈아가며 수행하여
		 * 		  마치 동시에 수행되는 것처럼 만드는 것
		 * -멀티쓰레딩으로 구현된 프로그램은 동작 속도가 빨라지는 것 보단
		 * 	 대기 없이 여러 작업을 번갈아가며 빠르게 수행하는데 중점을 둠
		 * 
		 * 
		 * <멀티 쓰레드 환경 구현 방법 2가지>
		 ...
		 * 		 이 떄, Runnable 인터페이스를 구현하는 클래스를 정의
...
//		mt2.run(); 
//		mt3.run(); 
		//=>run() 메서드를 직업 호출하는 것이 아니라
		//	   start() 메서드를 호출해야 멀티 쓰레딩이 수행됨
...
//		yt1.start(); //호출 불가능한 메서드(존재하지 않음)
		//반드시Thread 클래스의 객체를 톻애
		//Runnable 인터페ㅓ이스를 구현한 구현체 클래스 인스턴스를 전달하고
		//Thread객체의 start() 메서드를 호출해야 한다.
