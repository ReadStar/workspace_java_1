...
public class Ex {
	...
	public static void main(String[] args) {
		/*
		 * Object 클래스
		 * 2)toString() : 어떤 객체의 정보를 "문자열"로 리턴
		 * 	 	-일반적으로 개발자 입장에서 객체의 정보는 객체가 갖는
		 * 		  멤버변수들에 저장된 값을 의미하지만,
		 * 		  Object 클래스의 toString() 메서드는 객체의 멤버변수 값이 아닌
		 * 		  객체 고유의 값(주소값)을 문자열로 리턴해준다
		 * 		-따라서, 개발자가 원하는 멤버변수 값을 리턴받기 위해서는
		 * 		  toString() 메서드를 오버라이딩 하여,
		 ...
		 * 		-자동생성 단축기 = alt shif s => s
		 */
		Person p = new Person("홍길동", 20, "901010-1234567");
		System.out.println("Person클래스 객체 p 의 정보" + p.toString() );
		//=>객체 p가 갖는 정보를 문자열로 리턴받아 출력
		//		이 떄, 객체의 정보는 참조변수의 값이 아닌 고유값이 출력
		System.out.println("Person클래스 객체 p 의 정보" + p);
		...
		System.out.println("Person클래스 객체 p 의 정보" + p2.toString() );
		//출력문 내에서 toString()메서드는 생략가능
		System.out.println("Person클래스 객체 p 의 정보" + p2);
		
		//toString() 리턴값이 String 타입이므로 String 타입 변수 저장도 가능
		String info = p2.toString();
//		String info = p2; //변수에 toString() 메서드 리턴값 저장 시 toString() 생략 불가
		System.out.println(info);
		System.out.println("-------------------------------------------");
		//toString() 메서드가 오버라이딩 되어있는 API 예 : DATE 클래스
		//DATE 클래스 : 날자 정보를 관리하는 클래스
		Date d1 = new Date();
		//날자 정보를 출력문을 사용하여 출력
